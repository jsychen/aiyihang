  ...
  <div class="page-loadmore">
    <div class="page-loadmore-wrapper" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
      <mt-loadmore
        ...
        ref="loadmore"
      >
        <div class="wraps" v-for="item in list" :key="item.id" @click="next(item.id)">
          <div class="item">
            <div class="right">
              <div class="desc">{{item.name}}</div>
              <!--<div class="top">次卡</div>-->
            </div>
            <div class="center">
              <div class="top">可洗{{item.washNum-item.washedNum}}次</div>

              <!--<div class="money">免费</div>-->

              <div class="left">有效期：{{item.takeTime | timeparse}}-{{item.expireTime | timeparse}}</div>
            </div>
          </div>
...
.wraps {
  ...
  font-size: px2rem(18px);
  color: black;
  background: url(../../assets/chejuan@2.png) no-repeat;
  background-size: 100%;
  width: px2rem(355px);
  height: px2rem(200px);
}
